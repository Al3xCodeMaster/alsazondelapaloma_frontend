...
export default function Restaurants() {
  ...
  const updateSchedule = () => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "updateSchedule", {
      method: "POST",
      body: JSON.stringify({
  ...
  const set_open_DH = (idRes) => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "getAllRestaurantSchedules/" + idRes + "/" + true, {
      method: "GET",
    })
  ...
  const changeStateRest = (valueId, state) => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "updateRestaurant", {
      method: "POST",
      body: JSON.stringify({
  ...
  const performSave = () => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "createRestaurant", {
      method: "POST",
      body: JSON.stringify({
        RestaurantName: nombre_restaurant,
        RestaurantLatitude: coordenadas.lat,
  ...
  const refresh = () => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "listRestaurants/50/0", {
      method: "GET",
    })
      .then((res) => res.json())
      .then((response) => {
  ...
  useEffect(() => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "listSchedules/50/0", {
      method: "GET",
    })
      .then((res) => res.json())
      .then((response) => {
  ...
  const saveSchedule = () => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "createSchedule", {
      method: "POST",
      body: JSON.stringify({
  ...
  const scheduleRestaurant = (schedId, status) => {
    fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "assignScheduleToRestaurant", {
      method: "POST",
      body: JSON.stringify({
