  ...
  const handleClickShowPassword = () => {
    ...
    const perfomChange = (event) => {
        if(window.confirm("¿Desea cambiar la contraseña?")){
          fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "updateUserPassword", {
            method: 'POST',
            body: JSON.stringify({
    ...
    const changeProfilePic = (file) => {
      if(window.confirm("¿Desea subir la foto? con nombre de archivo:\n\n\t"+file.name)){
          ...
          fetch((process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "updatePhoto", {
            method: 'POST',
            body: formData
          }).then(res => res.json())
            .then(response => {
                ...
                <Grid item xs={6}>
                  <Badge
                        ...
                        badgeContent={<Tooltip placement="right-start" title="Subir/Cambiar foto"><IconButton aria-label="Cambiar foto" component="label"><CameraAltIcon/><Input
                                                                                                                                                                                ...
                                                                                                                                                                                /></IconButton></Tooltip>}
                                                                                                                                                                                >
                        <Avatar src={usuario.status==200?(process.env.URL_HEROKU_BACKEND || "http://localhost:4000/") + "file/"+usuario.userInfo.Payload.UrlPhoto:null } className={classes.avatar}/> 
                  </Badge>
